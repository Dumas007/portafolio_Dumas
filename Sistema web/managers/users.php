...
// OBTENER ESTADÍSTICAS PARA LAS CARDS - SOLO USUARIOS CON EL MISMO ROL
$stats = [];

// Total de técnicos - SOLO MISMO ROL
$query_total = "SELECT COUNT(*) as total FROM technicians WHERE role = :user_role";
$stmt_total = $db->prepare($query_total);
$stmt_total->bindParam(':user_role', $current_user_role);
$stmt_total->execute();
$stats['total_technicians'] = $stmt_total->fetchColumn();

// Técnicos activos - SOLO MISMO ROL
$query_active = "SELECT COUNT(*) as active FROM technicians WHERE role = :user_role AND active = 'yes'";
$stmt_active = $db->prepare($query_active);
$stmt_active->bindParam(':user_role', $current_user_role);
$stmt_active->execute();
$stats['active_technicians'] = $stmt_active->fetchColumn();

// Técnicos con datos faciales - SOLO MISMO ROL
$query_facial = "SELECT COUNT(*) as facial FROM technicians WHERE role = :user_role AND facial_data IS NOT NULL AND facial_data != '' AND facial_data != 'NULL'";
$stmt_facial = $db->prepare($query_facial);
$stmt_facial->bindParam(':user_role', $current_user_role);
$stmt_facial->execute();
$stats['technicians_with_face'] = $stmt_facial->fetchColumn();

// Distribución por estado - SOLO MISMO ROL
$query_status = "SELECT active, COUNT(*) as count FROM technicians WHERE role = :user_role GROUP BY active";
$stmt_status = $db->prepare($query_status);
$stmt_status->bindParam(':user_role', $current_user_role);
$stmt_status->execute();
$stats['status_distribution'] = $stmt_status->fetchAll(PDO::FETCH_ASSOC);
...
$technician = [
    ...
    'pass' => '',
    'facial_data' => '',
    'role' => $current_user_role, // Forzar el rol del usuario logueado
    'active' => 'yes'
];

$action = 'create';
$page_title = 'Usuarios - ' . htmlspecialchars($current_user_role);

// CORRECCIÓN: Primero procesar las acciones GET antes del POST
// Eliminar técnico - SOLO PERMITIR ELIMINAR USUARIOS DEL MISMO ROL
if(isset($_GET['delete']) && !empty($_GET['delete'])){
    $delete_id = filter_var($_GET['delete'], FILTER_VALIDATE_INT);
    
    // Verificar que solo pueda eliminar usuarios del mismo rol
    $query_check = "SELECT id, role FROM technicians WHERE id = :id";
    $stmt_check = $db->prepare($query_check);
    $stmt_check->bindParam(':id', $delete_id);
    $stmt_check->execute();
    $user_to_delete = $stmt_check->fetch(PDO::FETCH_ASSOC);
    
    if (!$user_to_delete || $user_to_delete['role'] !== $current_user_role) {
        $_SESSION['error_message'] = "No tiene permisos para eliminar este usuario";
    ...
    if ($delete_id == $current_user_id) {
        $_SESSION['error_message'] = "No puede eliminar su propio usuario desde aquí. Use la opción de eliminar cuenta en su perfil.";
        header("Location: users.php");
        exit();
    }
    
    $query = "DELETE FROM technicians WHERE id = :id AND role = :user_role";
    $stmt = $db->prepare($query);
    $stmt->bindParam(':id', $delete_id);
    $stmt->bindParam(':user_role', $current_user_role);
...
// Editar técnico - SOLO PERMITIR EDITAR USUARIOS DEL MISMO ROL
if(isset($_GET['edit']) && !empty($_GET['edit'])){
    $action = 'update';
    $page_title = 'Editar Usuario - ' . htmlspecialchars($current_user_role);
    
    // Validar que solo pueda editar usuarios del mismo rol
    $edit_id = filter_var($_GET['edit'], FILTER_VALIDATE_INT);
    
    $query = "SELECT * FROM technicians WHERE id = :id AND role = :user_role";
    $stmt = $db->prepare($query);
    $stmt->bindParam(':id', $edit_id, PDO::PARAM_INT);
    $stmt->bindParam(':user_role', $current_user_role, PDO::PARAM_STR);
    
    if($stmt->execute()){
        $technician = $stmt->fetch(PDO::FETCH_ASSOC);
        if(!$technician){
            $_SESSION['error_message'] = "Usuario no encontrado o no tiene permisos para editarlo";
            header("Location: users.php");
            exit();
        }
        ...
        $_SESSION['error_message'] = "Error al cargar el usuario";
        header("Location: users.php");
        exit();
    }
}

// Procesar formulario POST - SOLO PERMITIR ACTUALIZAR USUARIOS DEL MISMO ROL
if($_POST && $_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    // VERIFICAR QUE SOLO PUEDA ACTUALIZAR USUARIOS DEL MISMO ROL
    if(isset($_POST['id']) && !empty($_POST['id'])){
        // Actualizar usuario/técnico
        $action = 'update';
        $technician['id'] = $_POST['id'];
        
        // Validar que solo pueda actualizar usuarios del mismo rol
        $technician_id = filter_var($technician['id'], FILTER_VALIDATE_INT);
        
        // Verificar que el usuario a editar tenga el mismo rol
        $query_check = "SELECT id, role FROM technicians WHERE id = :id";
        $stmt_check = $db->prepare($query_check);
        $stmt_check->bindParam(':id', $technician_id);
        $stmt_check->execute();
        $user_to_edit = $stmt_check->fetch(PDO::FETCH_ASSOC);
        
        if (!$user_to_edit || $user_to_edit['role'] !== $current_user_role) {
            $_SESSION['error_message'] = "No tiene permisos para actualizar este usuario";
            header("Location: users.php");
            exit();
        }
        ...
        if(empty($technician['pass'])) {
            $query = "UPDATE technicians SET 
                      ...
                      active = :active
                      WHERE id = :id AND role = :user_role";
        } else {
        ...
        // CREAR NUEVO USUARIO - SOLO CON EL MISMO ROL
        if(empty($technician['pass'])) {
            $_SESSION['error_message'] = "La contraseña es obligatoria para nuevos usuarios";
            header("Location: users.php");
            exit();
        }
    ...
    try {
        ...
        if(isset($technician['id'])){
            ...
            // Solo bindear user_role si está en la consulta UPDATE
            if(strpos($query, ':user_role') !== false) {
                $stmt->bindParam(':user_role', $current_user_role, PDO::PARAM_STR);
            }
        }
        ...
        // Solo bindear role si está en la consulta (INSERT)
        if(strpos($query, ':role') !== false) {
            $stmt->bindParam(':role', $technician['role']);
        }
        
        // Solo bindear password si se está creando o actualizando con nueva contraseña
        if(($action == 'create' || !empty($technician['pass'])) && strpos($query, ':pass') !== false) {
...
// Obtener lista de técnicos - SOLO USUARIOS CON EL MISMO ROL
$query = "SELECT * FROM technicians WHERE role = :user_role ORDER BY name";
$stmt = $db->prepare($query);
$stmt->bindParam(':user_role', $current_user_role);
$stmt->execute();
$technicians = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Obtener roles para el select - SOLO EL ROL ACTUAL
$roles = [$current_user_role];
?>
...
<head>
    ...
    <title>Gestión de Usuarios - <?php echo htmlspecialchars($current_user_role); ?></title>
...
<body>
    ...
    <div class="admin-container">
        ...
        <div class="stats-grid">
            <div class="stat-card total">
                ...
                <div class="stat-subtext">Con rol <?php echo htmlspecialchars($current_user_role); ?></div>
            </div>
            ...
            <div class="stat-card active">
                ...
                <div class="stat-subtext"><?php echo round(($stats['active_technicians'] / max($stats['total_technicians'], 1)) * 100, 1); ?>% del rol</div>
            </div>
            ...
            <div class="stat-card facial">
                ...
                <div class="stat-label">CON RECONOCIMIENTO FACIAL</div>
                <div class="stat-subtext"><?php echo round(($stats['technicians_with_face'] / max($stats['total_technicians'], 1)) * 100, 1); ?>% del rol</div>
            </div>
        ...
        <div class="card">
            <div class="card-header">
                <h3><i class="fas fa-user-cog"></i> Información de Usuario - Rol: <?php echo htmlspecialchars($current_user_role); ?></h3>
            </div>
            <div class="card-body">
                <form method="POST" action="users.php" id="technicianForm">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            <label for="pass"><i class="fas fa-lock"></i> Contraseña <?php echo $action == 'create' ? '*' : ''; ?></label>
                        ...
                        <div class="form-group">
                            ...
                            <textarea id="facial_data" name="facial_data" class="form-control textarea-monospace" 
                                      rows="6" 
                                      placeholder="Datos de reconocimiento facial (embedding)"
                                      maxlength="65535" readonly><?php 
                        ...
                        <div class="form-group">
                            ...
                            <div class="password-note">
                                Todos los usuarios creados tendrán este rol <?php echo htmlspecialchars($current_user_role); ?>">
                            </div>
                        </div>
        ...
        <!-- Lista de Técnicos DEL MISMO ROL -->
        <div class="card">
            <div class="card-header">
                <h3><i class="fas fa-list"></i> Usuarios con Rol: <?php echo htmlspecialchars($current_user_role); ?></h3>
            </div>
            <div class="card-body">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td style="font-weight: 600;">
                                        <?php echo htmlspecialchars($tech['id']); ?>
                                        <?php if($tech['id'] == $current_user_id): ?>
                                            <span class="current-user-indicator" title="Usted">TÚ</span>
                                        <?php endif; ?>
                                    </td>
                                    ...
                                    <td>
                                        <span class="badge badge-primary">
                                            <?php echo htmlspecialchars($tech['role']); ?>
                                        </span>
                                    </td>
                                    <td>
                ...
                <?php else: ?>
                    <div class="empty-state">
                        <i class="fas fa-users"></i>
                        <h3>No hay usuarios con el rol <?php echo htmlspecialchars($current_user_role); ?></h3>
    ...
    <footer style="
        ...
        margin-top: 1rem;
    ">
        <div>Created by Erick Dumas &copy; <?php echo date('Y'); ?> | v1.0.0 | Rol: <?php echo htmlspecialchars($current_user_role); ?></div>
    </footer>
