...
if(!isset($_SESSION['technician_id']) || $_SESSION['technician_role'] != 'Admin'){
    ...
    error_log("Intento de acceso no autorizado a reports.php desde IP: " . $_SERVER['REMOTE_ADDR']);
...
if (!empty($startDate) && !empty($endDate)) {
    $whereConditions[] = "DATE(creation_date) BETWEEN ? AND ?";
    $queryParams[] = $startDate;
    $queryParams[] = $endDate;
} elseif (!empty($startDate)) {
    $whereConditions[] = "DATE(creation_date) >= ?";
    $queryParams[] = $startDate;
...
// Filtro por departamento (con validación)
if ($departmentFilter != 'all') {
    // Sanitizar entrada del departamento
    $departmentFilter = htmlspecialchars(strip_tags($departmentFilter));
...
if ($statusFilter != 'all') {
    if (!in_array($statusFilter, $allowed_statuses)) {
        $statusFilter = 'all'; // Valor por defecto si no es válido
    } else {
...
// Filtro por urgencia (con validación)
if ($urgencyFilter != 'all') {
    if (!in_array($urgencyFilter, $allowed_urgencies)) {
        $urgencyFilter = 'all'; // Valor por defecto si no es válido
    } else {
        $whereConditions[] = "urgency = ?";
        $queryParams[] = $urgencyFilter;
    }
}
...
try {
    ...
    // 6. Tiempo promedio de resolución (ACTUALIZADA)
    ...
    // 9. Horas pico de creación de tickets
    ...
    // 10. Tickets por día de la semana
    $sql = "SELECT 
    ...
    // 11. Análisis de eficiencia por departamento
    $sql = "SELECT 
            department,
            COUNT(*) as total,
    ...
    // 13. Tiempo promedio por urgencia (ACTUALIZADA)
    $sql = "SELECT 
            urgency,
    ...
    // 14. Intervalos de tiempo detallados con departamento y estatus (ACTUALIZADA)
    $timeIntervalsSql = "SELECT 
                        id,
                        query_id,
                        need,
                        department,
    ...
    $timeIntervalsStmt = $db->prepare($timeIntervalsSql);
    $timeIntervalsStmt->execute($queryParams);
    $time_intervals = $timeIntervalsStmt->fetchAll(PDO::FETCH_ASSOC);

    // Verificar si hay datos para mostrar en el análisis de tiempos
    if ($statusFilter != 'all' && $statusFilter != 'closed') {
        $check_times_sql = "SELECT COUNT(*) as has_times 
                           FROM tickets 
                           " . $whereClause . " 
                           AND update_date IS NOT NULL
                           AND update_date > creation_date";
        $check_times_stmt = $db->prepare($check_times_sql);
        $check_times_stmt->execute($queryParams);
        $has_times = $check_times_stmt->fetch(PDO::FETCH_ASSOC)['has_times'];
        $show_time_analysis = ($has_times > 0);
    }

} catch (PDOException $e) {
    // Manejo seguro de errores de base de datos
    error_log("Error en consulta de reportes: " . $e->getMessage());
    // Inicializar métricas vacías para evitar errores
    $metrics = [
        'total_tickets' => 0,
        'by_department' => [],
        'by_status' => [],
...
// Validar y sanitizar datos para evitar inyección XSS
...
<body>
    ...
    <div class="print-header">
        ...
        <div class="print-info">
            ...
            <?php 
                $filters = [];
                if ($departmentFilter != 'all') $filters[] = "Department: " . sanitize_output($departmentFilter);
    ...
    <nav class="admin-nav">
        ...
        <a href="reports.php"><i class="fas fa-chart-bar"></i> Coversations Dashboard</a>
        <a href="reports.php" class="active"><i class="fas fa-chart-bar"></i> Tickets Dashboard </a>
    </nav>

    <div class="admin-container">
        <div class="page-header">
            <h2>Ticket Metrics Dashboard</h2>
        ...
        <div class="insight-card">
            <div class="insight-title">
                <i class="fas fa-lightbulb"></i> Key Ticket System Insights
            </div>
            <p>The system shows a closure rate of <strong><?php echo sanitize_output($metrics['closure_rate']); ?>%</strong> 
                    ...
                    <div class="filter-group">
                        ...
                        <select class="filter-select" id="departmentFilter" name="departmentFilter">
                            ...
                            <?php foreach($metrics['by_department'] as $dept): ?>
                                <option value="<?php echo sanitize_output($dept['department']); ?>" 
                                    <?php echo $departmentFilter == $dept['department'] ? 'selected' : ''; ?>>
                                    <?php echo sanitize_output($dept['department']); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 2rem; margin-bottom: 2rem;">
                    ...
                    <div style="text-align: center;">
                        <div style="font-size: 2.5rem; font-weight: bold; color: var(--info); margin-bottom: 0.5rem;">
                            <?php echo round(($most_frequent_dept['count'] / $metrics['total_tickets']) * 100, 1); ?>%
                        </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    ...
                                    <?php if($dept['department'] == $most_frequent_dept['department']): ?>
        ...
        <!-- PROCESSING TIME ANALYSIS - ACTUALIZADO -->
        <div class="card avoid-break">
            <div class="card-header">
                <h3><i class="fas fa-clock"></i> Processing Time Analysis</h3>
                <?php if (!$show_time_analysis && $statusFilter != 'all' && $statusFilter != 'closed'): ?>
                    <span class="badge badge-warning" style="margin-left: 1rem;">
                        <i class="fas fa-info-circle"></i> Limited Data
                    </span>
                <?php endif; ?>
            </div>
            <div class="card-body">
                <?php if (!$show_time_analysis && $statusFilter != 'all' && $statusFilter != 'closed'): ?>
                    <div class="alert alert-info">
                        ...
                        <strong>Information:</strong> Time analysis is optimized for closed tickets. 
                        For open or in-process tickets, only basic time metrics are available.
                    </div>
                <?php endif; ?>
                ...
                <div class="metrics-grid">
                    <div class="metric-item">
                        ...
                        <div class="metric-info">
                            <div class="metric-value">
                                <?php
                                // Calcular tiempo promedio de asignación basado en tickets con tiempos
                                $avg_assign_time = $metrics['avg_resolution_time'] > 0 ? round($metrics['avg_resolution_time'] * 0.2, 1) : 0;
                                echo sanitize_output($avg_assign_time); 
                                ?>h
                            </div>
                    ...
                    <div class="metric-item">
                        ...
                        <div class="metric-info">
                            <div class="metric-value">
                                <?php
                                // Calcular tiempo promedio de resolución (estimado como 80% del tiempo total)
                                $avg_resolution_time_detailed = $metrics['avg_resolution_time'] > 0 ? round($metrics['avg_resolution_time'] * 0.8, 1) : 0;
                                echo sanitize_output($avg_resolution_time_detailed); 
                                ?>h
                            </div>
                    ...
                    <div class="metric-item">
                        ...
                        <div class="metric-info">
                            <div class="metric-value">
                                <?php echo sanitize_output($metrics['avg_resolution_time']); ?>h
                            </div>
                ...
                <!-- Tabla de intervalos de tiempo -->
                <?php if(!empty($time_intervals)): ?>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            <?php foreach($time_intervals as $interval): 
                                // Calcular tiempos estimados (20% asignación, 80% resolución)
                                $estimated_assign = $interval['total_time'] * 0.2;
                                $estimated_resolution = $interval['total_time'] * 0.8;
                                
                                // Determinar clases para estatus y urgencia
                                $status_class = $interval['status'] == 'open' ? 'text-warning' : 
                                              ($interval['status'] == 'in_process' ? 'text-info' : 'text-success');
                                $status_text = $interval['status'] == 'open' ? 'Open' : 
                                             ($interval['status'] == 'in_process' ? 'In Process' : 'Closed');
                            ...
                            <tr>
                                ...
                                <td><?php echo sanitize_output($interval['query_id']); ?></td>
                                <td>
                                    <span class="badge badge-primary"><?php echo sanitize_output($interval['department']); ?></span>
                                </td>
                                <td>
                ...
                <div style="text-align: center; padding: 2rem; color: #666;">
                    ...
                    <p style="font-size: 0.9rem;">Tickets with complete update dates are required</p>
                </div>
                <?php endif; ?>

                <!-- Análisis por urgencia -->
                <?php if(!empty($metrics['time_by_urgency'])): ?>
                <div style="margin-top: 2rem;">
                    <h4 style="margin-bottom: 1rem; color: var(--primary);">Resolution Time by Urgency Level</h4>
                    <div class="table-container">
                        <table class="table">
                            <thead>
                                <tr>
                            ...
                            <tbody>
                                <?php foreach($metrics['time_by_urgency'] as $urgency_time): 
                                    $urgency_text = '';
                                    $urgency_class = '';
                                    $performance = '';
                                    
                                    switch($urgency_time['urgency']) {
                                        case 'high_urg':
                                            $urgency_text = 'High Urgency';
                                            $urgency_class = 'urgency-high';
                                            $performance = $urgency_time['avg_time'] <= 24 ? 'Excellent' : 
                                                          ($urgency_time['avg_time'] <= 48 ? 'Good' : 'Needs Improvement');
                                            break;
                                        case 'medium_urg':
                                            $urgency_text = 'Medium Urgency';
                                            $urgency_class = 'urgency-medium';
                                            $performance = $urgency_time['avg_time'] <= 48 ? 'Excellent' : 
                                                          ($urgency_time['avg_time'] <= 72 ? 'Good' : 'Needs Improvement');
                                            break;
                                        case 'low_urg':
                                            $urgency_text = 'Low Urgency';
                                            $urgency_class = 'urgency-low';
                                            $performance = $urgency_time['avg_time'] <= 72 ? 'Excellent' : 
                                                          ($urgency_time['avg_time'] <= 96 ? 'Good' : 'Needs Improvement');
                                            break;
                                    }
                                ?>
                                <tr>
                                    ...
                                    <td>
                                        <strong><?php echo $urgency_time['avg_time'] ? round($urgency_time['avg_time'], 1) : 'N/A'; ?>h</strong>
                                    </td>
                                    <td>
                                        <span class="<?php 
                                            echo $performance == 'Excellent' ? 'text-success' : 
                                                  ($performance == 'Good' ? 'text-warning' : 'text-danger'); 
                                        ?>">
                                            <i class="fas fa-<?php 
                                                echo $performance == 'Excellent' ? 'check-circle' : 
                                                      ($performance == 'Good' ? 'clock' : 'exclamation-triangle'); 
                                            ?>"></i>
                ...
                <div style="background: #f8f9fa; padding: 1.5rem; border-radius: 10px; margin-top: 2rem;">
                    <h4 style="margin-bottom: 1rem; color: var(--primary);">Performance Insights</h4>
                    <div class="metrics-grid">
                        <div class="metric-item" style="background: white;">
                            ...
                            <div class="metric-info">
                                <div class="metric-value">
                                    <?php
                                    // Calcular el porcentaje de tickets resueltos en menos de 24 horas
                                    try {
                                        $fastTicketsSql = "SELECT COUNT(*) as fast_tickets 
                                                         FROM tickets 
                                                         " . $whereClause . " 
                                                         AND update_date IS NOT NULL
                                                         AND update_date > creation_date
                                                         AND TIMESTAMPDIFF(HOUR, creation_date, update_date) <= 24";
                                        $fastTicketsStmt = $db->prepare($fastTicketsSql);
                                        $fastTicketsStmt->execute($queryParams);
                                        $fast_tickets = $fastTicketsStmt->fetch(PDO::FETCH_ASSOC)['fast_tickets'];
                                        
                                        $total_with_times_sql = "SELECT COUNT(*) as total 
                                                               FROM tickets 
                                                               " . $whereClause . " 
                                                               AND update_date IS NOT NULL
                                                               AND update_date > creation_date";
                                        $totalWithTimesStmt = $db->prepare($total_with_times_sql);
                                        $totalWithTimesStmt->execute($queryParams);
                                        $total_with_times = $totalWithTimesStmt->fetch(PDO::FETCH_ASSOC)['total'];
                                        
                                        $fast_percentage = $total_with_times > 0 ? round(($fast_tickets / $total_with_times) * 100, 1) : 0;
                                        echo $fast_percentage . '%';
                                    } catch (PDOException $e) {
                                        echo 'N/A';
                                    }
                                    ?>
                                </div>
                                <div class="metric-label">Resolved in ≤ 24h</div>
                            </div>
                        </div>
                        ...
                        <div class="metric-item" style="background: white;">
                            ...
                            <div class="metric-info">
                                <div class="metric-value">
                                    <?php
                                    // Calcular tickets pendientes más antiguos
                                    if(!empty($metrics['oldest_open'])) {
                                        $oldest_ticket = $metrics['oldest_open'][0];
                                        $days_pending = floor((time() - strtotime($oldest_ticket['creation_date'])) / (60 * 60 * 24));
                                        echo $days_pending;
                                    } else {
                                        echo '0';
                                    }
                        ...
                        <div class="metric-item" style="background: white;">
                            ...
                            <div class="metric-info">
                                <div class="metric-value">
                                    <?php
                                    // Calcular tendencia vs período anterior (simulado)
                                    $previous_period_avg = $metrics['avg_resolution_time'] * 1.1; // Simular 10% más lento
                                    $trend = (($previous_period_avg - $metrics['avg_resolution_time']) / $previous_period_avg) * 100;
                                    echo round($trend, 1) . '%';
                                    ?>
                                </div>
            ...
            <div class="card-body">
                ...
                <div style="background: linear-gradient(135deg, #667eea, #764ba2); color: white; padding: 2rem; border-radius: 10px; margin-bottom: 2rem;">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 1rem;">
                        ...
                        <div style="text-align: center;">
                            <div style="font-size: 2rem; font-weight: bold;"><?php echo round(($most_frequent_need['frequency'] / $metrics['total_tickets']) * 100, 1); ?>%</div>
                ...
                <div class="table-container">
                    <table class="table">
                        ...
                        <tbody>
                            <?php foreach($metrics['frequent_needs'] as $index => $need): ?>
                            <tr>
                                <td>
                                ...
                                <td>
                                    ...
                                    <div class="progress-bar">
                                        <div class="progress-fill" style="width: <?php echo min(100, ($need['frequency'] / $most_frequent_need['frequency']) * 100); ?>%"></div>
                                    </div>
                                </td>
                                <td>
                                    <span class="badge badge-info"><?php echo round(($need['frequency'] / $metrics['total_tickets']) * 100, 1); ?>%</span>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
                    ...
                    <div class="metric-item">
                        ...
                        <div class="metric-info">
                            <div class="metric-value">
                                ...
                                $high_urgent = 0;
                                foreach($metrics['by_urgency'] as $urgency) {
                                    if($urgency['urgency'] == 'high_urg') {
                                        $high_urgent = $urgency['count'];
                                        break;
                                    }
                                }
                                echo sanitize_output($high_urgent);
                                ?>
                            </div>
    ...
    <div class="print-footer">
        <div>Report generated by Query System | Page <span class="page-number"></span></div>
    ...
    <script>
        ...
        const ChartManager = {
            ...
            init: function() {
                ...
                console.log('Charts initialized correctly');
            },
            ...
            destroyAll: function() {
                ...
                // Destroy global Chart.js instances
            ...
            createCharts: function() {
                ...
                if (deptCtx && !deptCtx.chart) {
                    this.instances.department = new Chart(deptCtx, {
                        type: 'bar',
                        data: {
                    ...
                    deptCtx.chart = this.instances.department;
                }
                ...
                if (timelineCtx && !timelineCtx.chart) {
                    ...
                    timelineCtx.chart = this.instances.timeline;
                }
                ...
                if (hourlyCtx && !hourlyCtx.chart) {
                    ...
                    hourlyCtx.chart = this.instances.hourly;
                }
                ...
                if (statusCtx && !statusCtx.chart) {
                    const statusMap = {
                        ...
                        'in_process': { label: 'In Process', color: '#3498db' },
                        'closed': { label: 'Closed', color: '#27ae60' }
                    };
                    
                    const statusLabels = <?php echo json_encode($status_js); ?>;
                    const statusData = <?php echo json_encode($status_count_js); ?>;
                    ...
                    statusLabels.forEach((status, index) => {
                        if (statusData[index] > 0) {
                            ...
                            filteredLabels.push(statusInfo.label);
                            filteredData.push(statusData[index]);
                            filteredColors.push(statusInfo.color);
                        }
                    });
                    ...
                    statusCtx.chart = this.instances.status;
                }

                // 5. Urgency Chart
                const urgencyCtx = document.getElementById('urgencyChart');
                if (urgencyCtx && !urgencyCtx.chart) {
                    const urgencyLabels = <?php echo json_encode($urgency_js); ?>;
                    const urgencyData = <?php echo json_encode($urgency_count_js); ?>;
                    
                    const translatedLabels = urgencyLabels.map(label => {
                        switch(label) {
                            case 'high_urg': return 'High Urgency';
                            case 'medium_urg': return 'Medium Urgency';
                            case 'low_urg': return 'Low Urgency';
                    ...
                    urgencyCtx.chart = this.instances.urgency;
                }

                // 6. Weekday Chart
                const weekdayCtx = document.getElementById('weekdayChart');
                if (weekdayCtx && !weekdayCtx.chart) {
                    ...
                    weekdayCtx.chart = this.instances.weekday;
                }
            },
        ...
        function resetDates() {
            if (confirm('Are you sure you want to clear the date filters?')) {
        ...
        document.getElementById('startDate')?.addEventListener('change', function() {
            ...
            if (this.value && endDate.value && this.value > endDate.value) {
                if (confirm('Start date is greater than end date. Do you want to adjust them automatically?')) {
        ...
        document.getElementById('endDate')?.addEventListener('change', function() {
            ...
            if (this.value && startDate.value && this.value < startDate.value) {
                if (confirm('End date is less than start date. Do you want to adjust them automatically?')) {
